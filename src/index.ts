  ...
  fetch(request: Request, env: Env, ctx: ExecutionContext) {
    ...
    return issuer({
      allow: async (input, req) => {
        let noSubdomain = (hostname: string) =>
          hostname.split(".").slice(1).join(".");
        let redirect_hostname = new URL(
          url.searchParams.get("redirect_uri") ?? ""
        ).hostname;
        let redirect_hostname_nosub = noSubdomain(redirect_hostname);
        return (
          redirect_hostname == "localhost" ||
          redirect_hostname_nosub == noSubdomain(url.hostname) ||
          redirect_hostname_nosub == "gorro-rojo.workers.dev"
        );
      },
